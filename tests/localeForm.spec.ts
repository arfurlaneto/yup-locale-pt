...
describe('locale', () => {
  ...
  it('should return localized messages for string group', async () => {
    ...
    try {
      ...
      expect(err.errors[0]).toStrictEqual('O campo deve ter exatamente 1 caractere.');
      expect(err.errors[1]).toStrictEqual('O campo deve ter pelo menos 1 caractere.');
      expect(err.errors[2]).toStrictEqual('O campo deve ter no máximo 1 caractere.');
      expect(err.errors[3]).toStrictEqual('O campo deve ter exatamente 5 caracteres.');
      expect(err.errors[4]).toStrictEqual('O campo deve ter pelo menos 5 caracteres.');
      expect(err.errors[5]).toStrictEqual('O campo deve ter no máximo 5 caracteres.');
      expect(err.errors[6]).toStrictEqual('O campo deve corresponder ao padrão: "/\\d+/".');
      expect(err.errors[7]).toStrictEqual('O campo deve ser um e-mail válido.');
      expect(err.errors[8]).toStrictEqual('O campo deve ser uma URL válida.');
      expect(err.errors[9]).toStrictEqual('O campo não deve conter espaços adicionais no início nem no fim.');
      expect(err.errors[10]).toStrictEqual('O campo deve estar em letras minúsculas.');
      expect(err.errors[11]).toStrictEqual('O campo deve estar em letras maiúsculas.');
    }
  });
  ...
  it('should return localized messages for array group', async () => {
    ...
    try {
      ...
      expect(err.errors[0]).toStrictEqual('O campo deve ter pelo menos 1 item.');
      expect(err.errors[1]).toStrictEqual('O campo deve ter no máximo 1 item.');
      expect(err.errors[2]).toStrictEqual('O campo deve ter pelo menos 5 itens.');
      expect(err.errors[3]).toStrictEqual('O campo deve ter no máximo 5 itens.');
    }
  });
});
